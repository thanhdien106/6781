...
// Изменяем сигнатуру на void __user **
...
void susfs_try_umount(uid_t target_uid) {
    ...
    // ВАЖНО: Добавь блокировку, если она используется в других частях для этого списка
    spin_lock(&susfs_spin_lock);

    // Размонтируем в обратном порядке (reversed order), это правильно для вложенных монтирований
    list_for_each_entry_reverse(cursor, &LH_TRY_UMOUNT_PATH, list) {
        
        // В твоем KernelSU функция называется try_umount и принимает (путь, флаги)
